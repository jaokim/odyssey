...
bool RenderThemeBal::paintMenuListButtonDecorations(const RenderBox& renderer, const PaintInfo& paintInfo, const FloatRect& r)
{
    ...
    // the control.
    Path menuListRoundedRectangle = roundedRectForBorder(renderer, r);

    // 1. Paint the background of the entire control.
    Color fillColor = renderer.style().visitedDependentColor(CSSPropertyBackgroundColor);
    ...
    IntRect bounds = IntRect(r.x() + renderer.style().borderLeftWidth(),
                         r.y() + renderer.style().borderTopWidth(),
                         r.width() - renderer.style().borderLeftWidth() - renderer.style().borderRightWidth(),
                         r.height() - renderer.style().borderTopWidth() - renderer.style().borderBottomWidth());

    drawArrowsAndSeparator(renderer, paintInfo, bounds);
